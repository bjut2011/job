  ...
  def Login
    ...
    if @user
      ...
      respond_to do |format|
         format.json {render :json => {:Status =>"0",:userInfo => {:userID => @user.id,:userName =>@user.name ,:smsemail => "",:sex =>"0",:mobile => "13810139056",:picurl => "",:signature=>"",:hasdevice =>@hasDevice},:resultState =>200,:errorMsg => "密码不对",:accountId =>"12345678"}}
      end
    else
     respond_to do |format|
         format.json {render :json => {:code =>1,:resultState =>200,:errorMsg => "密码不对",:accountId =>"12345678"}}
     end
    end
  end
