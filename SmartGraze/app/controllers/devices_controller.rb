class DevicesController < ApplicationController
  ...
  def GetDeviceDetail
    ...
    respond_to do |format|
      format.json { render :json => {:Status =>"0",:Msg =>"ok",:info => @device} }
    end
  end
 
  def GetAddressByLatlng
    uri = URI("http://api.map.baidu.com/geocoder/v2/?output=json&ak=Q15HFmKZ9NEPV8UjvItUNLE2&location="+params[:Lat]+","+params[:Lng])
    ...
    respond_to do |format|
      format.json { render :json => {:Status =>"0",:msg =>"ok",:address => city} }
    end
  ...
  def GetDeviceListResult
    
    @devices=User.find(params[:UserID]).device
    respond_to do |format|
      format.json { render :json => {:Status =>"0",:msg =>"ok",:GuardianList => @devices} }
    end
  end

  def GetTrackingByUserID
    @devices=Device.where(:id => params[:DeviceID])
    logger.info params[:DeviceID]
    respond_to do |format|
      format.json { render :json => {:Status =>"0",:msg =>"ok",:devices => @devices} }
    end
  end
