class AlarmsController < ApplicationController
  ...
  def alarmlogs
    if params["pid"]  and current_admin and  current_admin.type!=0
       ...
       @count=Alarmlogv.where(:code => { '$exists'=> true }).count
       @totolpage=@count/10+1;
       @alarmlogs=Alarmlogv.where(:code => { '$exists'=> true }).paginate(page:params[:page],per_page:10)
    end
  ...
  def addAlarms
     ...
     alarm=Alarmv.new(contactId:contactId,deviceId:deviceId,sensorId:sensorId,alarmType:alarmType,duration:duration,upperBoundC:upperBoundC,lowerBoundC:lowerBoundC,alarmTypeDiv:upperBoundC,target:target,switchVal:switchVal)
     alarm.save
