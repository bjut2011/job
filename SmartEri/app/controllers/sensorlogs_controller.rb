class SensorlogsController < ApplicationController
  ...
  def querySensorHistoryDatas
    ...
    logger.info params[:sensorid]
    dayt = Time.new(day.year, day.month, day.day)
    tm=dayt.to_i
    logger.info tm
    #if tt.year==2017 && tt.month==2
    #   @sensorlog=Sensorlog201702.where(:sensor_id => params[:sensorid],:update_time.gt => tm).sort(:time => :desc)
    #elsif  tt.year==2017 && tt.month==1
    #   @sensorlog=Sensorlog201701.where(:sensor_id => params[:sensorid],:update_time.gt => tm).sort(:time => :desc)
    #   logger.info "empty"
    #elsif  tt.year==2017 && tt.month==3
    #   @sensorlog=Sensorlog201703.where(:sensor_id => params[:sensorid],:update_time.gt => tm).sort(:time => :desc)
    #end
    #if @sensorlog.empty?
    #  params[:sensorid]="584a70ab97139613f33ec0d5"
    #  logger.info "empty"
    #end
    ...
    if type.nil?
      ...
      if tt.year==2017 && tt.month==2
        ...
        @sensorlog=Sensorlog201703.where(:sensor_id => params[:sensorid],:update_time.gt => tm).sort(:time => :desc)
      elsif  tt.year==2017 && tt.month==4
        @sensorlog=Sensorlog201704.where(:sensor_id => params[:sensorid],:update_time.gt => tm).sort(:time => :desc)
      elsif  tt.year==2017 && tt.month==5
        @sensorlog=Sensorlog201705.where(:sensor_id => params[:sensorid],:update_time.gt => tm).sort(:time => :desc)
      end
      logger.info params[:sensorid]
    elsif type.to_i==1
      day=day-1.hour
