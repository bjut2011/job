  ...
  def create_login_session
    ...
    if @user
      if @user.password_digest==params[:password]
        ...
        respond_to do |format|
           format.json {render :json => {:code =>0,:msg =>"登录成功",:redirect_uri =>"/main?pid="+@user.id}}
        end
      else
        respond_to do |format|
         format.json {render :json => {:code =>1,:msg =>"登录成功",:errorMsg => "密码不对",:redirect_uri =>"/main?pid="+@user.id}}
        end
