...
class AccessNode < ActiveRecord::Base
  ...
  def self.update_portal_url(params)
    ...
    if times<=0 or times>5
      {:check=>false, :code=>102, :msg=>"Execced Max Number"}
    elsif params[:url].nil? or !access=self.find_by_mac(params[:mac])
      {:check=>false, :code=>104,:msg=>"Not Found AccessNode"}
    else
        url = params[:url].lstrip  
      begin
        access.update_attributes!(redirect_url:url) 
      rescue Exception => e
        {:check=>false,:code=>103, :msg=>"Insert Error #{e.to_s}"}
      end
      {:check=>true,:code=>200,:msg=>"Success"}
  ...
  def self.update_welcome_url(params)
    times = 1
    ...
    if times<=0 or times>5
      ...
      begin
        url = params[:url].lstrip
        access.update_attributes!(portal_url:url) 
      rescue Exception => e
        {:check=>false,:code=>103, :msg=>"Insert Error #{e.to_s}"}
      end
      {:check=>true,:code=>200,:msg=>"Success"}
...
#     end
  ...
  def self.login(params)
    node = self.find_by_mac(params[:gw_id]) 
    guest = Guestnode.where("access_node_id = ?  ", node.id).first
    mac=""
    ...
    wx =  Weixin_Friend.where("guest_id = ? and weixin_id = ? and public_id = ? ",guest_id, weixin_id ,public_id).first
    bfriend=false
    if wx
       if wx.isFriend==1
          bfriend=true
    ...
    if bweixin
      if wx
        ...
        wx=Weixin_Friend.create!(:mac => mac,:weixin_id => weixin_id,:isFriend => 1,:ap_mac => node.mac, :public_id => public_id,:guest_id => guest_id)
      end
      begin
    ...
    if bweixin
       ...
       redirect_url ||= "http://#{params[:gw_address]}:#{params[:gw_port]}/ctbrihuang/auth?token=#{token}&url=www.baidu.com"
    end
  ...
  def self.login_zj(params)
    ...
    public_id=""
    if params[:url]
       logger.info "111111111111112222222222"
       if params[:url].include? "client_type"
          bweixin=true
    ...
    wx =  Weixin_Friend.where("guest_id = ? and  isFriend = 1 and mac = ? ",guestid,mac).first
    if bweixin
      wx =  Weixin_Friend.where("guest_id = ? and  weixin_id = ? and public_id = ? and mac = ?",guestid, weixin_id ,public_id,mac).first
    end
    bfriend=false
    if wx
       if wx.isFriend==1
    ...
    wx_clients =  Weixin_Friend.where("guest_id = ? and  isFriend = 1 and mac = ? ",guestid,mac)
    if wx_clients
      wx_clients.each do |wx_client|
          w2ap = Weixin_Ap.where(" guest_id = ? and weixin_publicid = ?", guestid,wx_client.public_id )
          if w2ap
             bfriend=true
             begin
              strurl=node.portal_url
              strurl="http://61.142.80.142/eapp/action/wifinote/save?"
              if strurl
                if strurl.index("?")
                    uri = URI.parse(strurl)
                    if !uri.query.blank?
              ...
              strurl+="mac="+mac
              strurl+="&weixin_openid="+wx_client.weixin_id
              strurl+="&public_id="+wx_client.public_id
              logger.info  strurl
              resp = Net::HTTP.get_response(URI(strurl))
             end
             rescue
              logger.info "error111111111"
             end
          end
      end
    end
    if bweixin
      if wx
        ...
        wx=Weixin_Friend.create!(:mac => mac,:weixin_id => weixin_id,:isFriend => 1, :guest_id => guest.guest_id, :public_id => public_id, :ap_mac => node.mac)
      end
      begin
       strurl=node.portal_url
       strurl="http://61.142.80.142/eapp/action/wifinote/save?"
