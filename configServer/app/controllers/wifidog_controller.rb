...
class WifidogController < ApplicationController
  ...
  def set_access_control_headers
    ...
    headers['Access-Control-Request-Method'] ="POST,GET,DELETE,PUT,OPTIONS"
  end
  ...
  def login
    ...
    respond_to do |format|
      if params[:callback]
       ...
       auth_url=""
       if node.auth_plattype==1
           auth_url="/ctbrihuang/auth?token=#{token}"
        else
          if node.auth_plattype==2
            auth_url="/smartwifi/auth?token=#{token}"

          else
            auth_url= "/ctbrihuang/auth?token=#{token}"
          end
       end
       format.json {render :json => {:result =>"OK",:token => token,:auth_url => auth_url}}
      end
    end
  end
