...
class AccessNodesController < ApplicationController
  before_filter :check_admin,:except => [:weixintoap,:getregisted]
  ...
  def getregisted
      ...
      c_admin = Guest.find_by_name(params[:name])
      if c_admin
        @access_nodes = AccessNode.where(:guest_id =>c_admin.id).paginate(page: page);
      end
      respond_to do |format|
        format.json {render :json => {:code=>"200",:aps=>@access_nodes}}
      end
  end
  ...
  def weixintoap
     ...
     if node
        if params[:weixin_openid]
          w2ap = Weixin_Ap.where(" mac= ? and weixin_publicid = ?", params[:mac],params[:weixin_openid] ).first
          logger.info node.guest_id
          logger.info "111tttt"
          if w2ap.nil?
            Weixin_Ap.create!(mac:params[:mac],weixin_publicid:params[:weixin_openid],weixin_publicname:params[:weixin_publicname],guest_id:node.guest_id)
          end       
        end
        respond_to do |format|  
          format.html { render :text=>"{code:0,message:\"OK\"}" }
        end
     else
       respond_to do |format|
        format.html { render :text=>"{code:0,message: \"mac is not null or not exist\"}" }
       end
     end
  end
