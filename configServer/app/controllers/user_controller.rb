...
class UserController < ApplicationController
  ...
  layout "special",:only=>[:portal_auto]
  ...
  def ioswificheck
      redirect_to "http://wifi.smbiztalk.cn/welcome/ioswificheck.php"
  end
  ...
  def defriending
      wxs =  Weixin_Friend.where("weixin_id = ? and public_id = ? and mac = ?",params[:weixin_openid] ,params[:public_id],params[:mac])
      wxs.each do |wx|
  ...
  def tag
     if params[:callback]
       str= params[:callback]+"({message:\"OK\",code:\"0\"})"
       respond_to do |format|
          format.html {render text: str}
       end
     else
       range = (1..3).to_a
       tagtype=range.sample(1)
       #tagtype=1
       str = "{\"tags\":[{\"tag\":#{tagtype}}],\"message\":\"OK\",\"code\":\"0\"}"
       pjson = JSON.parse str 
       respond_to do |format|
          format.json {render :json => pjson} 
       end
