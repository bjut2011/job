...
class FirewallController < ApplicationController
  ...
  def ip_add
    ...
    ipa = params[:ip].strip
    if @access.public_ips.count >= 100
      @err = "IP地址白名单已满"
    elsif ipa==""
      @err = "IP地址为空"
    else
    ...
    if !@err.nil?
     ...
     if @access.dev_id
         begin
           task = Task.where(" dev_id= ? and task_code = ? and status = ? ", @access.dev_id,"10000","0" ).first
           if task
  ...
  def ip_del
    ...
    pip=PublicIp.find(params[:id])
    access = AccessNode.find(pip.access_node_id)
    PublicIp.find(params[:id]).delete
    if access
       pips = PublicIp.where(" access_node_id= ? ", access.id )
       params = "{ \"whitelist\":["
       first =1
       pips.each do |pip|
         if first == 1
