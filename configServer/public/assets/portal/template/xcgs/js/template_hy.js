...
var MyTemplate = function(){
	...
	var getMaxWidth = function(){
		...
		_this.find('div#siteLoginModuleBox').css({width:PTPL_MAX_WIDTH});
		_this.find('div#Logo001').css({width:PTPL_MAX_WIDTH});
	}
	var onplaySlider = function(){
		var height = PTPL_MAX_WIDTH/2*2+80;
		var sl = $('div#siteSliderModuleBox').smallslider({width:PTPL_MAX_WIDTH,height:height,scale:MAX_SCALE});
	...
	var loadImageText = function(){
		...
		var width   = _this.find('div#siteImageTextModuleBox').width();
			...
			_this.find('div#siteImageTextModuleBox span').width(width-imwidth-40);
			...
			//_this.find('div#sitedevModuleBox img').css({width:imwidth,height:imwidth});
			_this.find('div#sitedevModuleBox ul li div').css({height:imwidth+20});
			_this.find('div#sitedevModuleBox dd').width(width-imwidth-31);
			//_this.find('div#sitedevModuleBox span').width(width-imwidth-31);
			if(width-liwidth-31<160){
				_this.find('div#sitedevModuleBox span').fadeOut();
			}else{
				_this.find('div#sitedevModuleBox span').fadeIn();
			}
	}
	
	var playOverlyer = function(){		
		if (!LOAD_OVERLY && !CUT_THUMB) {
			var thisimage = _this.find('div#siteLoadingModuleBox img')[0];
			...
			if (thisimage.width > 100 && thisimage.height > 100) {				
				if (thisimage.width / thisimage.height >= _width / _height) {
					if (thisimage.width > _width) {
						l_width = _width;
						l_height = (thisimage.height * _width) / thisimage.width;
					} else {
						l_width = thisimage.width;
						l_height = thisimage.height;
					}

				} else {
					if (thisimage.height > _height) {
						l_height = _height;
						l_width = (thisimage.width * _height) / thisimage.height;
					} else {
						l_width = thisimage.width;
						l_height = thisimage.height;
					}
			...
			if(!isNotEmptyString(l_width)){
				
			}
			$('div.portalTemplateBody .siteModules').css("visibility","hidden").eq(0).css("visibility","visible");
			_this.find('div#siteLoadingModuleBox').css({width:l_width,height:l_height,left:(PTPL_MAX_WIDTH-l_width)/2}).loadOverly();
			setTimeout(function(){$('div.portalTemplateBody .siteModules').css("visibility","visible");},6000);
			// _this.find('div#siteLoadingModuleBox').css({width:PTPL_MAX_WIDTH,height:_this.height()}).loadOverly();
		}else{
			_this.find('div#siteLoadingModuleBox').css({height:200,position:'relative'});
			_this.find('div#siteLoadingModuleBox').find('.loadingTimeTip').hide();
		}
	}
	...
	var imageTJBlock = function(){
		if($('div#siteImageTJModule').length>0){
			...
			//$('div#siteImageTJModule img').height(width-90);				
		}
	}	
	var generateUserInfo = function(){
		if (!LOAD_OVERLY && !CUT_THUMB) {
			//$('div.bottombutton').css({'padding':0}).html('<button type="button" class="AuthSubmitID optionbutton" onclick="onValidatorOption()">点击上网</button>');
	...
	return {init:function(editmode){
		...
		if(editmode){
			...
			$('#siteLoginModuleBox').css({position:'relative'});			
		}		
...
$(document).ready(function(){
	...
	$(document).on('click', 'a#compeditdom', function(){
		if(!$(this).hasClass('disabled')){
			...
			if(__edit_moduleid=='siteLogoViewModuleBox'||__edit_moduleid=='siteSliderModuleBox'||__edit_moduleid=='siteImageSpanModule'||__edit_moduleid=='siteImageTextModuleBox'||__edit_moduleid=='siteImageTJModule'){
	...
	switch(id){
		...
		case "siteFooterModuleBox":
			...
			var data     = getPageModuleComponentList(__edit_sitepage.data, id);
				...
				html = $('<pre>').append(loadhtml.clone()).html();
			break;
		case "siteColumnModuleBox1":
			var loadhtml = $(showTextBlockModuleForm);
			var data     = getPageModuleComponentList(__edit_sitepage.data, id);
				if(data){
					for(i=0;i<data.length;i++){
						if(getComponentPermission('siteColumnModuleBox1', data[i].componentid)){
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}else{
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" class="disabled" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}
				...
				html = $('<pre>').append(loadhtml.clone()).html();
			break;
		case "siteColumnModuleBox2":
			var loadhtml = $(showTextBlockModuleForm);
			var data     = getPageModuleComponentList(__edit_sitepage.data, id);
				if(data){
					for(i=0;i<data.length;i++){
						if(getComponentPermission('siteColumnModuleBox2', data[i].componentid)){
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}else{
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" class="disabled" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}
				...
				html = $('<pre>').append(loadhtml.clone()).html();
			break;
		case "siteColumnModuleBox3":
			var loadhtml = $(showTextBlockModuleForm);
			var data     = getPageModuleComponentList(__edit_sitepage.data, id);
				if(data){
					for(i=0;i<data.length;i++){
						if(getComponentPermission('siteColumnModuleBox3', data[i].componentid)){
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}else{
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" class="disabled" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}
				...
				html = $('<pre>').append(loadhtml.clone()).html();
			break;
		case "siteColumnModuleBox4":
			var loadhtml = $(showTextBlockModuleForm);
			var data     = getPageModuleComponentList(__edit_sitepage.data, id);
				if(data){
					for(i=0;i<data.length;i++){
						if(getComponentPermission('siteColumnModuleBox4', data[i].componentid)){
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}else{
							var item = '<li><div class="cm-name">'+data[i].content.title+'</div><div>'+data[i].content.url+'</div><div>'+data[i].content.description+'</div><a href="javascript:;" class="disabled" componentid="'+data[i].componentid+'" id="compeditdom">编辑</a></li>';
						}
