...
void addNode(json_object *contentNodes,char* reSpec,double value);
void addNode_s(json_object *contentNodes,char* reSpec,char* value);
void addNode_i(json_object *contentNodes,char* reSpec,int value);
...
void insertSensorRecord_s( mongoc_collection_t * sensorcoll,bson_oid_t *de_oid,mongoc_collection_t * sensorlogcoll,char* value,char *name,int sensorType,char * unit,char *tag,int icon,int order,int display){
   ...
   //
    ...
    bson_t *update = BCON_NEW ("$set", "{", "value", BCON_UTF8(value),"tag",BCON_UTF8(tag),"updatetime",BCON_DOUBLE(timep),"icon",BCON_INT32(icon),"order",BCON_INT32(order),"display",BCON_INT32(display), "}");
...
{  
   ...
   strftime (buffer,sizeof(buffer),"sensorlog%Y%ms",timeinfo);
   printf("%s\n",buffer);
   ...
   sensorlogcoll = mongoc_client_get_collection (mgclient, "smart_trash_development", buffer);
   mongoc_cursor_t *cursor;
   const bson_t *doc;
   bson_t *query;
   char *str;
...
///
   ...
   char strgps[128];
   memset(strgps,'\0',128);
   sprintf(strgps,"%.04f,%.04f",fLongitude,fLatitude);
   insertSensorRecord_s(sensorcoll,&de_oid,sensorlogcoll,strgps,"经纬度",1,"度","正常",10,15,1);
   //
   bson_t reply;
   bson_error_t error;
   time_t timep;
...
void uppermqtt(unsigned char * info ,unsigned short usTemp_Length)
{  
   char *str;
   char phone[15];
   sprintf(phone,"%c%c%c%c%c%c%c%c%C%C%C",info[80],info[81],info[82],info[83],info[84],info[85],info[86],info[87],info[88],info[89],info[90]);
   printf("phone:%s\n",phone);
   if((strcmp(phone,"10649000016")!=0) && (strcmp(phone,"64913128315")!=0)){
      return;
   }
   int ifind=0;
...
///
    ...
    */
    char pGateWay[1024];
    memset(pGateWay,'\0',1024);
    strcpy(pGateWay,"CTBRI$smartTrashCan");
    strcat(pGateWay,phone);
    ...
    json_object *sourceGateway = json_object_new_string(pGateWay);
    ...
    time_t rawpretime;
    struct tm *timePtr;
    struct tm *timePrePtr;

    time( &rawtime );
    rawpretime=rawtime -60;
    timePtr = localtime( &rawtime );
    timePrePtr = localtime( &rawpretime );
    char timeNextString[80];
    strftime(timeNextString, 80,"%Y-%m-%dT%XZ", timePrePtr);
    char timeDate[80];
    strftime(timeDate, 80,"%Y-%m-%d", timePtr);
    char timeTime[80];
    strftime(timeTime, 80,"%X", timePtr);
    char timeString[80];
    getCurrentTime(timeString);
    json_object *contentNodes = json_object_new_array();
    addNode_s(contentNodes,"CTBRI$cIMSIResSpec",phone);
    addNode_s(contentNodes,"CTBRI$cDateResSpec",timeDate);
    addNode_s(contentNodes,"CTBRI$cTimeResSpec",timeTime);
    addNode_i(contentNodes,"CTBRI$uImportanceResSpec",1);
    addNode_s(contentNodes,"CTBRI$uMainboardIDResSpec","MTK");
    addNode_i(contentNodes,"CTBRI$uMainboardStatusResSpec",1);
    addNode_i(contentNodes,"CTBRI$uBatteryStatusResSpec",1);
    addNode_i(contentNodes,"CTBRI$uSensorStatusResSpec",1);
    addNode_s(contentNodes,"CTBRI$cMainBoardIDResSpec","MTK");
    addNode(contentNodes,"CTBRI$uTodayDoorOpenTimesResSpec",iOpenNumT);
    addNode(contentNodes,"CTBRI$uToday70PercentCompressTimesResSpec",iStartNumT);
    addNode(contentNodes,"CTBRI$uToday90PercentCompressTimesResSpec",iNumT);
    addNode(contentNodes,"CTBRI$uMainBatteryVoltageResSpec",iMainBatteryVoltage);
    addNode(contentNodes,"CTBRI$uSubBatteryVoltageResSpec",iSecondaryBatteryVoltage);
    addNode(contentNodes,"CTBRI$uTemperatureResSpec",iTemperature);
    addNode_s(contentNodes,"CTBRI$cGPSCurrentTimeResSpec",timeString);
    addNode_s(contentNodes,"CTBRI$cLongitudeEWResSpec","E");
    addNode_i(contentNodes,"CTBRI$cLongitudeIntResSpec",(int)fLatitude);
    addNode(contentNodes,"CTBRI$cLongitudeDecimalResSpec",fLatitude-(int)fLatitude);
    addNode_s(contentNodes,"CTBRI$cLatitudeNSResSpec","N");
    addNode_i(contentNodes,"CTBRI$cLatitudeIntResSpec",(int)fLongitude);
    addNode(contentNodes,"CTBRI$cLatitudeDecimalResSpec",fLongitude-(int)fLongitude);
    addNode_i(contentNodes,"CTBRI$cDisplacementResSpec",0);
    addNode_i(contentNodes,"CTBRI$cCurrentDisplacementLimitResSpec",20);
    addNode_s(contentNodes,"CTBRI$cLastGPSTimeResSpec",timeNextString);
    addNode_s(contentNodes,"CTBRI$cLastLongitudeEWResSpec","E");
    addNode_i(contentNodes,"CTBRI$cLastLongitudeIntResSpec",(int)fLatitude);
    addNode(contentNodes,"CTBRI$cLastLongitudeDecimalResSpec",fLatitude-(int)fLatitude);
    addNode_s(contentNodes,"CTBRI$cLastLatitudeNSResSpec","N");
    addNode_i(contentNodes,"CTBRI$cLastLatitudeIntResSpec",(int)fLongitude);
    addNode(contentNodes,"CTBRI$cLastLatitudeDecimalResSpec",fLongitude-(int)fLongitude);
    json_object_object_add(result,"contentNodes", contentNodes);

    const char *cPayload = json_object_to_json_string(result);
    //MQTTClient curClient=getMClient();
    MQTTClient curClient;
    MQTTClient_message pubmsg = MQTTClient_message_initializer;
    MQTTClient_deliveryToken token;
    MQTTClient_connectOptions conn_opts = MQTTClient_connectOptions_initializer;
    int rc;

    MQTTClient_create(&curClient, ADDRESS, "CLIENTIDempp",
    ...
    if ((rc = MQTTClient_connect(curClient, &conn_opts)) != MQTTCLIENT_SUCCESS)
    {
        printf("Failed to connect, return code %d\n", rc);
        return NULL;
    }

    pubmsg.payload = (void *) cPayload;
    pubmsg.payloadlen = strlen(cPayload);
    pubmsg.qos = QOS;
    pubmsg.retained = 0;
    MQTTClient_publishMessage(curClient, TOPIC, &pubmsg, &token);
    rc = MQTTClient_waitForCompletion(curClient, token, TIMEOUT);
    printf("mqtt publish:%s\n",cPayload);
    json_object_put(result);
    MQTTClient_disconnect(curClient, 10000);
    MQTTClient_destroy(&curClient);
}
...
void addNode(json_object *contentNodes,char* reSpec,double value){
    ...
    */
    ...
    json_object *resourceSpec1 = json_object_new_string(reSpec);
    ...
    /*
        "value": 23.5
    */
    json_object *contentNode1Value = json_object_new_double(value);
...
void addNode_s(json_object *contentNodes,char* reSpec,char* value){
    ...
    */
    ...
    /*
        "value": 23.5
    */
    json_object *contentNode1Value = json_object_new_string(value);
    json_object_object_add(contentNode1, "value", contentNode1Value);
...
void addNode_i(json_object *contentNodes,char* reSpec,int value){
    ...
    */
    ...
    /*
        "value": 23.5
    */
    json_object *contentNode1Value = json_object_new_int(value);
    json_object_object_add(contentNode1, "value", contentNode1Value);
