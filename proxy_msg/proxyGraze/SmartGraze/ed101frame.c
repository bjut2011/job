...
unsigned char VerifyIED101Frame(unsigned char * pTemp_Buf, unsigned short usTemp_Length, unsigned char ucTemp_Port,char* device_moblie,unsigned char * device_ge){
    int i=0;
    ...
    char *tv=(char *)malloc(sizeof(char)*(20));
    memset(tv,'\0',20);
    char *ver=(char *)malloc(sizeof(char)*(20));
    memset(ver,'\0',20);
    for(;i<usTemp_Length;i++){
    ...
    if (strlen(platlon) || 1==1){
      upperquery(pid,platlon,device_moblie,device_ge,tv,ver);
    }
    printf("moblie:%s\n",device_moblie);
...
void commond_parse(unsigned char code,unsigned char * pTemp_Buf,int start,int end, char * pout){
      ...
      if(code=='F'){
        ...
        printf("GPS 整数:%d,%d",lat,lon);
        double dlat=0.0;
        double dlon=0.0;
        if (lat/100000.0<1000){
             dlat=lat/10000.0;
             dlon=lon/10000.0;
        }else{
             dlat=lat/100000.0;
        ...
        printf("%f,%f\n",convertGPS(dlat),convertGPS(dlon));
        double glon=convertGPS(dlon);
        double glat=convertGPS(dlat);
        double glatlon[2];
        glatlon[0]=lat;
        glatlon[1]=lon;
      ...
      if(code=='H'){
        ...
        sprintf(pout,"%c,%c",pTemp_Buf[start+1],pTemp_Buf[start+2]);
        printf("\n");
      }
...
void upperquery(char * pid,char* platlon,char * device_moblie,unsigned char * device_ge,char * electricity,char * version)
{  
          ...
          if (bson_iter_init (&iter, doc)) {
            while (bson_iter_next (&iter)) {
               ...
               if (value->value_type == BSON_TYPE_OID) {
                 ...
                 if(strcmp(bson_iter_key (&iter),"device_moblie")==0){
                    strcpy(device_moblie,value->value.v_utf8.str);
                    printf ("%s\n", device_moblie);
                 }
               }else if(value->value_type == BSON_TYPE_INT32){
                 if(strcmp(bson_iter_key (&iter),"device_ge")==0){
                    ige=value->value.v_int32;
                    printf ("ge:%d\n",ige);
                 }
               }
   ...
   if(ige>0){
      device_ge[1] =  (unsigned char) ((ige>>8) & 0xFF);    
      device_ge[0] =  (unsigned char) (ige & 0xFF); 
   }
   bson_destroy (query);
   mongoc_cursor_destroy (cursor);
   if(bfind && strlen(platlon)>0){

  
     //
     char *slat=strtok(platlon,",");
     char *slon=strtok(NULL,",");
     printf("GPS:%s,%s",slat,slon);
     char sdatetime[128];
     memset(sdatetime,'\0',128);
     time_t timep;  
     struct tm *p;   
     ...
     bson_t *update = BCON_NEW ("$set", "{", "baiduLat", BCON_UTF8(slat), "baiduLng",BCON_UTF8(slon),"serverUtcDate",BCON_UTF8(sdatetime), "version",BCON_UTF8(version), "electricity",BCON_UTF8(electricity),"}");
