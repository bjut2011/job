    ...
    while(1)
      {
        ...
        BYTE device_moblie[56];
        memset(device_moblie,'\0',56);
        BYTE device_ge[2];
        memset(device_ge,'0',2);
        if(data_recv[0]==0x7B){
          ...
          printf("101\n");
          VerifyIED101Frame(data_recv,i_recvBytes,0,device_moblie,device_ge);
          data_send[0]=0x0B;
          printf("device_moblie:%s\n",device_moblie);
          printf("device_ge:%02x%02x\n",device_ge[1],device_ge[0]);
        }
        ...
        if(strlen(device_moblie)>0){
          char cmd_C[256];
          memset(cmd_C,'\0',256);
          strcat(cmd_C,"C,");
          strcat(cmd_C,device_moblie);
          strcat(cmd_C,";");
          int c_i=0;
          for(;c_i<strlen(cmd_C);c_i++){
             data_cmd[next]=(int)cmd_C[c_i];
             next++;
          }
        }
        ...
        next++;
        //data_cmd[next]=0x00;
        data_cmd[next]=device_ge[1];
        next++;
        data_cmd[next]=device_ge[0];
        next++;
        data_cmd[next]=',';
        next++;
        //data_cmd[next]=0x00;
