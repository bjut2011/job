...
static void getaddress(char *msg,char * lanlng);
...
static void do_geofence(mongoc_collection_t *geocoll,bson_oid_t g_oid,mongoc_collection_t *fence2devicecoll,mongoc_collection_t *devicecoll,mongoc_collection_t *logcoll,double lat,double lng,int type, double radius);
...
static void do_alarm_e(mongoc_collection_t *devicecoll,bson_oid_t de_oid,mongoc_collection_t *alarmlogcoll,double electricity);
int sockfd;  
...
void updateAlarm(mongoc_collection_t *coll,bson_oid_t se_oid,int type,double lat, double lng){
    ...
    bson_t *update=NULL;
    if(type==3){
      update = BCON_NEW ("$set", "{", "alarmtime_e",BCON_DOUBLE(timep), "type",BCON_INT32(type),"lat",BCON_DOUBLE(lat),"lng",BCON_DOUBLE(lng),"}");
    }else{
      update = BCON_NEW ("$set", "{", "alarmtime",BCON_DOUBLE(timep), "type",BCON_INT32(type),"lat",BCON_DOUBLE(lat),"lng",BCON_DOUBLE(lng),"}");
    }
...
alarm_send(void)
{
   	...
   	while (mongoc_cursor_next (cursor, &doc)) {
   	  ...
   	  bson_oid_t se_oid;
          if (bson_iter_init (&iter, doc)) {
            while (bson_iter_next (&iter)) {
               ...
               if (value->value_type == BSON_TYPE_UTF8) {
                 ...
                 if(strcmp(bson_iter_key (&iter),"radius")==0){
                    radius=atof(value->value.v_utf8.str);
                 }
               }
          ...
          do_geofence(geopolycoll,se_oid,fence2devicecoll,collection,logcoll,lat,lng,type,radius);
          char mobile[256];
          char addr[1024];
          memset(mobile,'\0',256);
          memset(addr,'\0',1024);
   	  ...
   	  bson_oid_t se_oid;
          if (bson_iter_init (&iter, doc)) {
            while (bson_iter_next (&iter)) {
               ...
               if (value->value_type == BSON_TYPE_UTF8) {
                 if(strcmp(bson_iter_key (&iter),"electricity")==0){
                    electricity=atof(value->value.v_utf8.str);
                 }
               }
               if (value->value_type == BSON_TYPE_INT32) {
                 if(strcmp(bson_iter_key (&iter),"type")==0){
          ...
          if((electricity<11)&&(electricity>0.0))
            do_alarm_e(collection,se_oid,logcoll,electricity);
   ...
   if(buse ==1){
         ...
         if(is_inside(pts,NN,pt)==0){
           ...
           if(bsend){
             ...
             char latlng[128];
             memset(latlng,'\0',128);
             sprintf(latlng,"%f,%f",lat,lng);
             char straddress[1024];
             memset(straddress,'\0',1024);
             getaddress(straddress,latlng);
             time_t timep;
             struct tm *p;   
             time(&timep);   
             p = localtime(&timep);
             ...
             char info[2048];
             memset(info,'\0',2048);
             sprintf(info,"\"{\\\"Type\\\":\\\"InGeofence\\\", \\\"Data\\\":{\\\"Msg\\\":\\\"出电子围栏\\\",\\\"Type\\\":\\\"102005\\\",\\\"BLat\\\":\\\"%f\\\",\\\"BLon\\\":\\\"%f\\\",\\\"Address\\\":\\\"%s\\\",\\\"DateTime\\\":\\\"%s\\\",\\\"DeviceID\\\":\\\"%s\\\"}}\"",lat,lng,straddress,sdatetime,de_id);
             char user[128];
             memset(user,'\0',128);
             sprintf(user," -a %s",user_id);
             char cmd[4096];
             memset(cmd,'\0',4096);
             strcpy(cmd,"python ");
             strcat(cmd," /root/Xg/XgMessage.py -m ");
             strcat(cmd,info);
             strcat(cmd,user);
             printf(cmd);
             system(cmd);
...
static void do_alarm_e(mongoc_collection_t *devicecoll,bson_oid_t de_oid,mongoc_collection_t *alarmlogcoll,double electricity){
   ...
   while (mongoc_cursor_next (cursor, &doc)) {
          ...
          if (bson_iter_init (&iter, doc)) {
            while (bson_iter_next (&iter)) {
               ...
               if (value->value_type == BSON_TYPE_DOUBLE) {
                 if(strcmp(bson_iter_key (&iter),"alarmtime_e")==0){
                    gtime=value->value.v_double; 
                 }
               }
               if (value->value_type == BSON_TYPE_OID) {
   ...
   if(buse ==1){
         if(electricity<11){
           ...
           if(bsend){
             int code=sendsms_c(buf,mobile);
             updateAlarm(devicecoll,de_oid,3,lat,lng);
             saveAlarmLog(alarmlogcoll,de_id,code,"",msg,user_id,lat,lng);
             ...
             char info[2048];
             memset(info,'\0',2048);
             sprintf(info,"\"{\\\"Type\\\":\\\"InGeofence\\\", \\\"Data\\\":{\\\"Msg\\\":\\\"低电量报警\\\",\\\"Type\\\":\\\"101016\\\",\\\"BLat\\\":\\\"%f\\\",\\\"BLon\\\":\\\"%f\\\",\\\"Address\\\":\\\"%s\\\",\\\"DateTime\\\":\\\"%s\\\",\\\"DeviceID\\\":\\\"%s\\\"}}\"",lat,lng,straddress,sdatetime,de_id);
             char user[128];
             memset(user,'\0',128);
             sprintf(user," -a %s",user_id);
             char cmd[4096];
...
static void getaddress(char *msg,char* lanlng){
    ...
    if (curl) {
      ...
      strcpy(strurl,"http://api.map.baidu.com/geocoder/v2/?ak=IDvNBsejl9oqMbPF316iKsXR&output=json&location=");
      strcat(strurl,lanlng);
